...
import { playersData } from "../../data/players2";
...
export function CreateTeam() {
  ...
  useEffect(() => {
    async function getupcoming() {
      if (id) {
        setLoading(true);
        const data = playersData; //await axios.get(`${URL}/getplayers/${id}`);
        console.log(data, "testdata");
        setLive(data.data.live);
        let awayPlayers = data.data.team_a_squad.map((obj) => ({
          ...obj,
          isHome: false,
          code: data.data.team_a_id,
        }));
        let homePlayers = data.data.team_b_squad.map((obj) => ({
          ...obj,
          isHome: true,
          code: data.data.team_b_id,
        }));
        if (!data.data.live) {
          if (state?.editMode) {
            ...
            setPlayers([
              ...p.map((r) =>
                state.selectedPlayers.find((f) => f.player_id == r.player_id)
                  ? { ...r, isSelected: true }
                  : r
              ),
            ]);
      ...
      {!next ? (
        <>
          ...
          <NextButtonContainer>
            <PrevButton
              onClick={() =>
                navigate(`/savedteam/${state?.teamId}`, {
                  state: {
